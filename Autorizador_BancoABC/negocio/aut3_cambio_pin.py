...
def procesar_aut3_cambio_pin(
    ...
    from seguridad.cifrado_aes import descifrar
    print("=== DEBUG CAMBIO PIN ===")
    print("PIN actual cifrado recibido:", request.get("PinActual"))
    print("PIN actual descifrado:", descifrar(request.get("PinActual")))
    print("PIN nuevo cifrado recibido:", request.get("PinNuevo"))
    print("PIN nuevo descifrado:", descifrar(request.get("PinNuevo")))
    print("CVV cifrado recibido:", request.get("CodigoDeVerificacion"))
    print("CVV descifrado:", descifrar(request.get("CodigoDeVerificacion")))
    print("Fecha cifrada recibida:", request.get("FechaDeVencimiento"))
    print("Fecha descifrada:", descifrar(request.get("FechaDeVencimiento")))
    print("Tarjeta encontrada:", tarjeta is not None)
    if tarjeta:
        print("PIN almacenado (cifrado):", tarjeta.get("TARJETA_PIN"))
        print("PIN almacenado (descifrado):", descifrar(tarjeta.get("TARJETA_PIN")))
        print("CVV almacenado (cifrado):", tarjeta.get("TARJETA_NumVerificacion"))
        print("CVV almacenado (descifrado):", descifrar(tarjeta.get("TARJETA_NumVerificacion")))
        print("Fecha almacenada (cifrada):", tarjeta.get("TARJETA_FechaVencimiento"))
        print("Fecha almacenada (descifrada):", descifrar(tarjeta.get("TARJETA_FechaVencimiento")))
    print("=========================")
