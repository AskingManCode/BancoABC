...
def procesar_aut1_retiro(
    ...
    pin_request = str(request.get("Pin", "")).strip()
    ...
    if monto > 50000:
        ...
        if not iguales_por_texto_descifrado(pin_request, str(tarjeta.get("TARJETA_PIN", "")).strip()):
            _bitacora("Retiro", cajero_id, numero_cif, cliente_id, "0.00", "")
            return resp("2")
    ...
    if tipo_id == 1:
        ...
        almacen_autorizaciones.guardar(
            codigo=codigo,
    ...
    # Crédito -> descontar de una vez en MySQL
    if tipo_id == 2:
        cuenta_id = int(tarjeta.get("CUENTA_ID", 0))
        tarjeta_id = int(tarjeta.get("TARJETA_ID", 0))

        resultado_bd = repo_trans.descontar_adelanto_credito_y_registrar_retiro(
            cuenta_id=cuenta_id,
            tarjeta_id=tarjeta_id,
            identificador_cajero=cajero_id,
            monto=monto
        )
