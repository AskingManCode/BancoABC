...
def procesar_aut5_confirmacion(
    ...
    if tipo_id == 2:
        codigo_sp = repo_trans.ejecutar_sp_aut5_adelanto_efectivo(
            numero_tarjeta_cifrado=numero_cif,
        ...
        # SP: 0 OK, 1 insuficiente, 2 datos incorrectos, 5 error
        if int(codigo_sp) == 0:
            return {"status": "OK"}
        if int(codigo_sp) in (1, 2, 5):
            return {"status": str(int(codigo_sp))}
        return {"status": "5"}
